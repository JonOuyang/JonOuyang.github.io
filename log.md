...
- **Mobile navbar adjustments**: Removed the floating bottom `<MobileDock />` overlay from the home page route in [App.jsx](src/App.jsx) while preserving the clean minimal top nav in [ChatPage.jsx](src/experimental/chat/ChatPage.jsx). The bottom dock remains active on subpages (`/work-history`, `/projects`, `/research`, etc.).
