...
const ProjectsPage = () => {
  ...
  const allProjects = rows[0]?.items || [];
  const researchItems = rows[1]?.items || [];

  // Simple category buckets using existing data
  const byIds = (ids) => allProjects.filter((p) => ids.includes(p.id));
  const fullstackProjects = byIds([3, 6, 8, 9, 2]).length ? byIds([3, 6, 8, 9, 2]) : allProjects.slice(0, 4);
